...
if [ ! -f "$GATEWAY/.env" ]; then
    warn "gateway/.env not found — copying from .env.example"
...
nohup uvicorn hooks:app --app-dir "$GATEWAY" --port 4001 > /tmp/tapiod-fastapi.log 2>&1 &
