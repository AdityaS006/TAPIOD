...
**Linux / macOS / WSL**
```bash
cp backend/.env.example backend/.env   # add GROQ_API_KEY=gsk_...
chmod +x scripts/setup.sh && ./scripts/setup.sh
```

**Windows**
```
copy backend\.env.example backend\.env   # add GROQ_API_KEY=gsk_...
scripts\setup.bat
```
...
| `gateway-redis` | 6379 | L1 exact-match cache |
...
This loads the 5,000 chatbot-arena prompts into the `routing_examples` Qdrant collection. The source file (`seeds/arena_prompts.json`) is already in the repo — this step just embeds and indexes it.
...
> **Note:** Re-running `seeds/seed_all.py` is safe — it clears and rebuilds the collection from scratch. It will re-download nothing (the embed model is cached after first run), but will re-embed all 5,000 prompts (~1–2 min).
...
## 6. Start the FastAPI server (port 4001)
...
| FastAPI server | 4001 | http://localhost:4001 |
...
→ Make sure you activated the venv: `source backend/venv/bin/activate`
...
→ Check `backend/.env` exists and contains `GROQ_API_KEY=gsk_...`
...
The provided `scripts\start.bat` launches LiteLLM + Next.js together but does **not** start the FastAPI server — run `uvicorn main:app --port 4001 --reload` in a separate terminal from the `backend/` directory.
