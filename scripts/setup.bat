...
cd /d "%ROOT%"
start "TAPIOD — LiteLLM Proxy :4000" cmd /k "cd /d "%ROOT%backend" && call venv\Scripts\activate && litellm --config litellm_config.yaml --port 4000"
timeout /t 3 /nobreak > nul
start "TAPIOD — FastAPI Gateway :4001" cmd /k "cd /d "%ROOT%backend" && call venv\Scripts\activate && uvicorn main:app --port 4001 --reload"
start "TAPIOD — Dashboard :3000" cmd /k "cd /d "%ROOT%frontend" && npm run dev"
